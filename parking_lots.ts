 ...
 */
export function make_parking_table(length: number, name: string): ParkingTable{ 
    const table: ParkingTable = { 
        name: name,
        spots: new Array(length), 
 ...
 * for testing that will not be available for the users 
 */
export function set_up(): void {
    //All parkinglots you can reserv a parking spot in
    let Ångströmslaboratoriet = make_parking_table(9, "angstromslaboratoriet"); 
    ...
    //Parkingspot users can not park at 
 ...
 */
export function save_parking_to_file(file: string, 
                                     parks: Array<ParkingTable>): void{
 ...
 */
export function load_parks_from_file(
                                file: string): Array<ParkingTable> | undefined{
    try {
 ...
 * @modifies the parks array with all parkinglots as well as a file 
 */
 ...
 *      returns undefined 
 */
export function get_park_from_parkingLots(
                    file: string, parkLot: string): ParkingTable | undefined {
